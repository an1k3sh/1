...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        if use_e_prompt and e_prompt_layer_idx is not None:
            ...
            _num_layers, _dual, _pool_size, _length, _num_heads, _embed_dim_divby_num_heads = self.e_prompt.prompt.shape
            #for i in range(_pool_size):
            #    embd_tsr = torch.load("./embed/clip_Split-CIFAR100/"+str(i)+".pt").to("cuda" if torch.cuda.is_available() else "cpu")
            #    embd_tsr = embd_tsr.squeeze(0)
            #    embd_tsr = torch.cat(embd_tsr, torch.zeros(embed_dim -embd_tsr.shape[0],), dim = 0)
            #    embd_tsr = embd_tsr.reshape(_num_heads, _embed_dim_divby_num_heads)
            #    self.e_prompt.prompt[:, :, i, :, :, :] = embd_tsr.expand(_num_layers, _dual, 1, _length, _num_heads, _embed_dim_divby_num_heads)
    ...
    def generate_prompt_indices(self, train = False, batch_num = -1):
        print('TM', train, batch_num)
        if self.prompt_length is None:
            return None
        if len(self.prompt_order_list) == batch_num:
            _ls = list(range(self.e_prompt.length))
            random.shuffle(_ls)
            print(_ls)
            self.prompt_order_list.append(_ls)
            #return self.prompt_order_list[batch_num][]
        if self.use_e_prompt and self.e_prompt_layer_idx is not None and train:
            return self.prompt_order_list[batch_num][:self.e_prompt.send_length]
        if self.use_e_prompt and self.e_prompt_layer_idx is not None:
            #return self.prompt_order_list[batch_num][:self.e_prompt.send_length]
            return list(range(self.e_prompt.length))
        return None

    def forward_features(self, x, task_id=-1, prompt_id=None, prompt_weight=None, train=False, prompt_momentum=0, batch_num = -1):
        outp = self.generate_prompt_indices(train, batch_num)
        prompt_list = None
        if outp is not None:
            prompt_list = sorted(outp)
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            if self.use_g_prompt or self.use_e_prompt:
                ...
                res = self.e_prompt(x, prompt_list, prompt_mask=prompt_mask, prompt_idx=prompt_id, prompt_weight=prompt_weight, prompt_momentum=prompt_momentum)
    ...
    def forward(self, x, task_id=-1, prompt_id=None, prompt_weight=None, train=False, fc_only=False, prompt_momentum=0, batch_num=-1):
        if fc_only:
            res = dict()
            x = self.mlp(x)
            x = self.fc_norm(x)
            res['logits'] = self.head(x)
            return res
        res = self.forward_features(x, task_id=task_id, prompt_id=prompt_id, prompt_weight=prompt_weight, train=train, prompt_momentum=prompt_momentum, batch_num=batch_num)
...
def vit_base_patch16_224_dino(pretrained=False, **kwargs):
    ...
    #del model.head
    state_dict = model.state_dict()
    ckpt = torch.load('/scratch/a_parashar.iitr/HiDe-Prompt/checkpoints/dino_vitbase16_pretrain.pth', map_location='cpu')
    not_in_k = [k for k in ckpt.keys() if k not in state_dict.keys()]
    for k in not_in_k:
        del ckpt[k]
    state_dict.update(ckpt)

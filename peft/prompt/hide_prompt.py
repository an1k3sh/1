...
class EPrompt(nn.Module):
    def __init__(self, length=5, embed_dim=768, embedding_key='mean', prompt_init='uniform', prompt_pool=False, 
        ...
        self.length = math.ceil(length * 1.5)
    ...
    def forward(self, x_embed, prompt_list, prompt_mask=None, prompt_idx=None, prompt_weight=None, prompt_momentum=0):
        ...
        if self.prompt_pool:
            ...
            if len(prompt_list) == 1:
                prompt_list = torch.tensor(prompt_list[0])
            #print(prompt_list)
            if self.use_prefix_tune_for_e_prompt:
                if prompt_weight is not None:
                    ...
                    batched_prompt_raw = batched_prompt_raw[:, :, :, :, prompt_list]
